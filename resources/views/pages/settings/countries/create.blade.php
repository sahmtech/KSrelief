...
@section('title', __('settings.countries.create_title'))

@section('content')
<x-page-header
    :title="__('settings.countries.create_title')"
    :subtitle="__('settings.countries.create_subtitle')"
    :breadcrumbs="[
        ...
        ['label' => __('settings.countries.create_title')],
    ]"
/>
...
<form method="POST" action="{{ route('settings.countries.store') }}">
    ...
    <x-card :title="__('settings.sections.details')">
        @include('pages.settings.countries.partials.form')
    </x-card>
    <div class="d-flex gap-2 mt-3">
        <button type="submit" class="btn btn-primary"><i class="ti ti-check me-1"></i> {{ __('common.create') }}</button>
        <a href="{{ route('settings.countries.index') }}" class="btn btn-outline-secondary">{{ __('common.cancel') }}</a>
    </div>
</form>
@endsection
