...
@section('title', __('settings.countries.title'))

@section('content')
<x-page-header
    :title="__('settings.countries.title')"
    :subtitle="__('settings.countries.subtitle')"
    :breadcrumbs="[
        ['label' => __('menu.settings')],
        ['label' => __('settings.dashboard.title'), 'url' => route('settings.dashboard')],
        ['label' => __('settings.countries.title')],
    ]"
>
    <x-slot:actions>
        @can('country.create')
            <a href="{{ route('settings.countries.create') }}" class="btn btn-primary btn-sm">
                <i class="ti ti-plus me-1"></i> {{ __('settings.countries.add') }}
            </a>
        @endcan
    </x-slot:actions>
</x-page-header>
...
<x-card :title="__('settings.filters.title')" :compact="true" class="mb-4">
    <form method="GET" action="{{ route('settings.countries.index') }}" class="row g-3 align-items-end">
        <div class="col-md-5">
        ...
        <div class="col-md-4 d-flex gap-2">
            ...
            <a href="{{ route('settings.countries.index') }}" class="btn btn-outline-secondary btn-sm">{{ __('settings.filters.reset') }}</a>
        </div>
    </form>
</x-card>
        ...
        <table class="table table-hover mb-0">
            <thead>
                <tr>
                    ...
                    <th>{{ __('settings.table.iso2') }}</th>
                    <th>{{ __('settings.table.iso3') }}</th>
                    <th>{{ __('settings.table.phone_code') }}</th>
            ...
            <tbody>
                        ...
                        <td class="text-end">
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end shadow border-0">
                                    @can('country.view')
                                        <li><a class="dropdown-item" href="{{ route('settings.countries.show', $country) }}"><i class="ti ti-eye me-2"></i>{{ __('settings.actions.view') }}</a></li>
                                    @endcan
                                    @can('country.update')
                                        <li><a class="dropdown-item" href="{{ route('settings.countries.edit', $country) }}"><i class="ti ti-pencil me-2"></i>{{ __('settings.actions.edit') }}</a></li>
                                    @endcan
                                    @can('country.delete')
                                        <li><hr class="dropdown-divider"></li>
                                        <li>
                                            <form method="POST" action="{{ route('settings.countries.destroy', $country) }}">
                                                @csrf
                                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="7" class="text-center text-muted py-4">{{ __('settings.messages.empty') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
