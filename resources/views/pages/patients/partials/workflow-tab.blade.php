{{-- Medical Workflow tab on patient profile --}}

<div class="row g-4">
    <div class="col-lg-4">
        <div class="card border-0 bg-light h-100">
            <div class="card-body text-center py-4">
                <p class="text-muted small mb-1">{{ __('workflow.current_stage') }}</p>
                @if($patient->currentStage)
                    <span class="badge rounded-pill fs-6 px-4 py-2 mb-2"
                          style="background-color: {{ $patient->currentStage->color ?? '#3B82F6' }}; color: #fff;">
                        {{ $patient->currentStage->name }}
                    </span>
                @else
                    <span class="badge bg-secondary rounded-pill fs-6 px-4 py-2 mb-2">
                        {{ __('workflow.no_stage') }}
                    </span>
                @endif
                <p class="text-muted small">
                    {{ __('workflow.last_updated') }}: {{ $patient->updated_at?->format('d M Y') }}
                </p>

                @can('changeStage', $patient)
                <button type="button" class="btn btn-primary btn-sm mt-1"
                        data-bs-toggle="modal" data-bs-target="#changeStageModal">
                ...
                <div class="mt-3 d-flex flex-column gap-2">
                    ...
                    <a href="{{ route('patients.workflow.history', $patient) }}" class="btn btn-sm btn-outline-secondary">
                        <i class="ti ti-history me-1"></i> {{ __('workflow.stage_history') }}
                    </a>
                    @endcan
                    @can('create', [\App\Models\MedicalRecord::class, $patient])
                    <a href="{{ route('patients.records.create', $patient) }}" class="btn btn-sm btn-outline-primary">
                        <i class="ti ti-file-plus me-1"></i> {{ __('workflow.records.add') }}
                    </a>
                    @endcan
                </div>
            </div>
    ...
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body py-4">
                <x-workflow-timeline :timeline="$workflowTimeline" />
            </div>
        </div>
    </div>
</div>
            ...
            <form action="{{ route('patients.workflow.change-stage', $patient) }}" method="POST">
                ...
                <div class="modal-header border-bottom">
                    <h5 class="modal-title" id="changeStageModalLabel">
                        <i class="ti ti-transfer me-2 text-primary"></i>{{ __('workflow.change_stage_modal.title') }}
                    </h5>
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label fw-semibold">
                            {{ __('workflow.change_stage_modal.new_stage') }} <span class="text-danger">*</span>
                        </label>
                        ...
                        @error('to_stage_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="mb-3">
                ...
                <div class="modal-footer border-top">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="ti ti-check me-1"></i>{{ __('workflow.change_stage_modal.confirm') }}
                    </button>
                </div>
            </form>
        </div>
