...
@php
    $displayName = $name ?? $patient?->patient_name ?? '';
    $imageUrl = $src ?? $patient?->photoUrl();
    $displayInitials = $initials ?? $patient?->initials() ?? (mb_strtoupper(mb_substr(trim($displayName), 0, 2)) ?: '?');
